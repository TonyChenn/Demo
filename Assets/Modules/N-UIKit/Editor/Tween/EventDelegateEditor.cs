...
using Entry = TweenKit.PropertyReferenceDrawer.Entry;
...
namespace TweenKit
{
    static public class EventDelegateEditor
    {
        ...
        static public bool Field(Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
        {
            ...
            {
                if (!minimalistic) TweenKitEditorTools.SetLabelWidth(82f);
            ...
            if (remove)
            {
                TweenKitEditorTools.RegisterUndo("Delegate Selection", undoObject);
                del.Clear();
                TweenKitTools.SetDirty(undoObject);
            }
            ...
            if (del.target != null && del.target.gameObject != null)
            {
                ...
                TweenKitEditorTools.DrawPadding();
                GUILayout.EndHorizontal();
                ...
                if (ps != null)
                {
                    for (int i = 0; i < ps.Length; ++i)
                    {
                        ...
                        if (selGO != null)
                        {
                            ...
                            int selection;
                            string[] props = GetNames(ents, TweenKitTools.GetFuncName(param.obj, param.field), out selection);
        ...
        static public string[] GetNames(List<Entry> list, string choice, out int index)
        {
            ...
            {
                Entry ent = list[i];
                string del = TweenKitTools.GetFuncName(ent.target, ent.name);
                names[++i] = del;
